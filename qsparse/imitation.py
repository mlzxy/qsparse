...
def imitate(
    ...
    """Patch the class of the input module, in order to in-place transform the
    weight / bias attributes of the input module.
    ...
    This mechanism enables us to have a much simpler API design than existing other prune/quantize libraries.
    ...
    We believe it is a minor issue because
        1. Only the state dict is pickled for most scenarios.
        2. We could use cloudpickle like `torch.save(module, path, pickle_module=cloudpickle)`, which supports closures.

    _Names of the arguments are inspired from the movie `The Thing`._
